...
const renderFilms = () => {   // Se declara una función anónima de flecha
  ...
  if (filteredFilms.length === 0) {
    app.innerHTML = `
      <div class="mb-3 d-flex align-items-center flex-wrap gap-2">
        <button id="btn-all-films" class="btn btn-secondary">Todas las peliculas</button>
        <button id="btn-available-films" class="btn btn-secondary">Top 5 actores</button>
  ...
  const buttons = `
    <div class="mb-3 d-flex align-items-center flex-wrap gap-2">
      <button id="btn-all-films" class="btn btn-secondary">Todas las películas</button>
      <button id="btn-available-films" class="btn btn-secondary">Top 5 actores</button>
      <div class="input-group" style="width: 350px;">
        <input type="text" id="search-input" class="form-control" placeholder="Buscar por título o descripción...">
        <button class="btn btn-primary" type="button" id="filter-button">
...
const renderPagination = () => {
  ...
  for (let i = startPage; i <= endPage; i++) {
    buttons += `<button class="btn ${i === currentPage ? 'btn-danger text-white fw-bold' : 'btn-light text-danger fw-bold'} me-1" data-page="${i}">${i}</button>`;
  }
